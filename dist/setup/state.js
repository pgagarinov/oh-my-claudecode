 ...
 */
export function saveState(step, configType, opts) {
 ...
 */
export function clearState(opts) {
    const cwd = opts?.cwd ?? process.cwd();
    const log = opts?.logger ?? NOOP_LOGGER;
    const stateFile = join(cwd, STATE_SUBPATH);
 ...
 */
export function resumeState(opts) {
    const cwd = opts?.cwd ?? process.cwd();
    const log = opts?.logger ?? NOOP_LOGGER;
    const stateFile = join(cwd, STATE_SUBPATH);
    if (!existsSync(stateFile)) {
    ...
    log(`Found previous setup session (Step ${lastStep} completed at ${parsed.timestamp}, configType=${configType})`);
    return {
        status: 'resume',
        lastStep,
 ...
 */
export function completeSetup(version, opts) {
    const cwd = opts?.cwd ?? process.cwd();
    ...
    const rawSid = process.env['CLAUDE_SESSION_ID'] ?? process.env['CLAUDECODE_SESSION_ID'] ?? '';
    if (rawSid && SESSION_ID_REGEX.test(rawSid)) {
        // Valid session ID: delete only that session's file
        const sessionFile = join(cwd, '.omc/state/sessions', rawSid, 'skill-active-state.json');
