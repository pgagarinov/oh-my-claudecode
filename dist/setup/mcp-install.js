...
export class McpCredentialMissingError extends Error {
    server;
    envVar;
    constructor(server, envVar) {
        super(`MCP server "${server}" requires credential ${envVar} but none was provided. `
...
// Public entry point
// ---------------------------------------------------------------------------
export async function installMcpServers(servers, creds, opts) {
    const result = {
        installed: [],
    ...
    const runExec = opts.execFile ?? execFileSync;
    ...
    for (const entry of servers) {
        ...
        try {
            ...
            if (action.withoutAuth) {
                ...
                log(`[mcp] installed ${entryName} WITHOUT credentials — visible in `
...
async function planInstall(entry, creds, opts, log) {
    if (entry === 'context7') {
        return {
            kind: 'install',
    ...
    if (entry === 'exa') {
        const key = await resolveSecret('EXA_API_KEY', creds.exa, 'Enter EXA_API_KEY (blank to skip): ', 'exa', opts);
        if (!key) {
    ...
    if (entry === 'github') {
        const token = await resolveSecret('GITHUB_TOKEN', creds.github, 'Enter GITHUB_PERSONAL_ACCESS_TOKEN (blank to skip): ', 'github', opts);
        if (token) {
            return {
                kind: 'install',
                args: [
...
function planCustom(entry, opts) {
...
// ---------------------------------------------------------------------------
async function resolveSecret(envVar, fromCreds, promptMessage, serverName, opts) {
    if (fromCreds && fromCreds.length > 0)
        return fromCreds;
...
// Command execution
// ---------------------------------------------------------------------------
function runClaudeMcpAdd(execFile, addArgs, scope) {
