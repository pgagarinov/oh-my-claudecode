 ...
 */
export function ensureLocalOmcGitExclude(opts = {}) {
...
export function resolveTargetPaths(mode, configDir, cwd = process.cwd()) {
    if (mode === 'local') {
 ...
 */
export function extractOldVersion(targetPath) {
...
export function backupIfExists(targetPath, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
    if (!existsSync(targetPath)) {
...
export async function loadCanonicalOmcContent(pluginRoot, opts = {}) {
    ...
    if (typeof fetchImpl !== 'function') {
        throw new Error(`Failed to download CLAUDE.md. Aborting.\nFALLBACK: Manually download from: ${url}`);
    }
    let response;
    try {
        ...
        throw new Error(`Failed to download CLAUDE.md from ${url}: ${err.message}\n` +
            `FALLBACK: Manually download from: ${url}`);
    }
    if (!response.ok) {
        throw new Error(`Failed to download CLAUDE.md from ${url}: HTTP ${response.status}\n` +
            `FALLBACK: Manually download from: ${url}`);
    }
 ...
 */
export function validateOmcMarkers(content, sourceLabel) {
    if (!content.includes(START_MARKER) || !content.includes(END_MARKER)) {
        throw new Error(`Canonical CLAUDE.md source is missing required OMC markers: ${sourceLabel}\n` +
 ...
 */
export function stripOmcMarkers(content) {
 ...
 */
export function ensureNotSymlinkPath(targetPath, label) {
 ...
 */
export function writeWrappedOmcFile(destination, omcContent) {
 ...
 */
export function ensureManagedCompanionImport(targetPath, companionName) {
...
export function mergeOmcBlock(existing, omcContent, version) {
    return mergeClaudeMd(existing, omcContent, version);
}
export function installPreserveMode(args) {
 ...
 */
export function migrateNoMarkers(existingContent, omcContent) {
 ...
 */
export function cleanupOrphanedCompanion(configDir, backupDate, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
 ...
 */
export function validatePostWrite(targetPath) {
    const content = readFileSync(targetPath, 'utf8');
    if (!content.includes(START_MARKER) || !content.includes(END_MARKER)) {
 ...
 */
export function reportVersionChange(oldVersion, newVersion, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
    if (oldVersion === 'none') {
 ...
 * (bash lines 396-402).
 */
export function cleanupLegacyHooks(configDir, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
    const legacyHooks = [
        'keyword-detector.sh',
 ...
 */
export function warnLegacyHooksInSettings(settingsPath, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
 ...
 */
export function reportPluginStatus(settingsPath, opts = {}) {
    const log = opts.logger ?? ((line) => console.log(line));
    if (existsSync(settingsPath)) {
        try {
...
export async function installClaudeMd(opts) {
    ...
    if (mode !== 'local' && mode !== 'global') {
        throw new Error(`Invalid mode '${mode}'. Use 'local' or 'global'.`);
    }
    if (installStyle !== 'overwrite' && installStyle !== 'preserve') {
        throw new Error(`Invalid install style '${installStyle}'. Use 'overwrite' or 'preserve'.`);
    }
