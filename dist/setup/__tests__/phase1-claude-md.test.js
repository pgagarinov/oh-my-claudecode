...
describe('runPhase1', () => {
    it('forwards target=local and installStyle=overwrite to installClaudeMd', async () => {
        ...
        const lines = [];
        const result = await runPhase1(makeOptions({ target: 'local', installStyle: 'overwrite' }), (line) => lines.push(line), { installClaudeMd: stub });
    ...
    it('forwards target=global and installStyle=preserve to installClaudeMd', async () => {
        ...
        const result = await runPhase1(makeOptions({ target: 'global', installStyle: 'preserve' }), () => { }, { installClaudeMd: stub });
        const call = stub.mock.calls[0][0];
    ...
    it('returns the minimal Phase1Result shape (no extra fields leaked)', async () => {
        ...
        const result = await runPhase1(makeOptions(), () => { }, {
            installClaudeMd: stub,
        });
        expect(Object.keys(result).sort()).toEqual(['backupPath', 'installStyle', 'mode', 'newVersion', 'oldVersion', 'targetPath'].sort());
    });
    it('propagates errors from installClaudeMd to the caller', async () => {
        ...
        await expect(runPhase1(makeOptions(), () => { }, { installClaudeMd: stub }))
...
//# sourceMappingURL=phase1-claude-md.test.js.map