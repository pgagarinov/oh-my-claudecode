...
function makeFakePrompter(script) {
    const calls = [];
    const writes = [];
    const secrets = [...(script.secrets ?? [])];
    const prompter = {
        async askSelect(question, options, defaultValue) {
            ...
            const firstWord = (s) => s.split(/[\s(—→]/).filter((p) => p.length > 0)[0] ?? s;
            const hit = options.find((o) => o.label === label || firstWord(o.label) === firstWord(label));
            const returned = (hit?.label ?? defaultValue);
            calls.push({
                kind: 'select',
                question,
                returned,
...
describe('runInteractiveWizard', () => {
    ...
    it('skipInstallCliQuestion=true: installCli question omitted, resolved as false', async () => {
        ...
        const installCliAsked = calls.some((c) => c.question.includes('install the OMC CLI globally'));
    ...
    describe('CLAUDE_CONFIG_DIR awareness', () => {
        ...
        it('Q1 target options show resolved absolute paths (CLAUDE_CONFIG_DIR set)', async () => {
            ...
            const targetCall = calls.find((c) => c.kind === 'select' && c.question.includes('Where should I configure'));
        ...
        it('Q2 installStyle options show resolved CLAUDE.md + companion paths', async () => {
            ...
            const q2Call = calls.find((c) => c.kind === 'select'
            ...
            expect(overwriteOpt?.label).toContain('/fixture/home/.claude-alt/CLAUDE.md');
            // Preserve LABEL must name BOTH base + companion paths.
            expect(preserveOpt?.label).toContain('/fixture/home/.claude-alt/CLAUDE.md');
            expect(preserveOpt?.label).toContain('/fixture/home/.claude-alt/CLAUDE-omc.md');
        });
...
//# sourceMappingURL=wizard-prompts.test.js.map