...
describe('runPhase3', () => {
    ...
    it('reports plugin verified when settings.json mentions oh-my-claudecode', async () => {
        writeFileSync(join(configDir, 'settings.json'), JSON.stringify({ plugins: { 'oh-my-claudecode': { enabled: true } } }), 'utf8');
        const lines = [];
    ...
    it('skips MCP install when mcp.enabled is false', async () => {
        ...
        const result = await runPhase3(makeOptions({ mcp: { ...DEFAULTS.mcp, enabled: false, credentials: {}, servers: [] } }), () => { }, {
            configDir,
    ...
    it('calls installMcpServers with scope=user when mcp.enabled=true', async () => {
        ...
        const lines = [];
        const result = await runPhase3(makeOptions({
            interactive: false,
            mcp: {
                enabled: true,
    ...
    it('writes teammateMode into settings.json when displayMode != auto', async () => {
        ...
        expect(settingsPatch['env']['CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS']).toBe('1');
    });
    ...
    it('starts from empty env when settings.json has no env key', async () => {
        writeFileSync(join(configDir, 'settings.json'), JSON.stringify({ theme: 'dark' }), 'utf8');
        const mergeSettings = vi.fn();
        await runPhase3(makeOptions({
            teams: {
                enabled: true,
    ...
    it('ignores a non-object settings.json.env to avoid crashing on malformed user files', async () => {
        writeFileSync(join(configDir, 'settings.json'), JSON.stringify({ env: 'not-an-object' }), 'utf8');
        const mergeSettings = vi.fn();
        await runPhase3(makeOptions({
            teams: {
                enabled: true,
...
//# sourceMappingURL=phase3-integrations.test.js.map