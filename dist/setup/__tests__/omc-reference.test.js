...
describe('installOmcReferenceSkill', () => {
    ...
    const makeSkillSource = (root, content = '# omc-reference skill\n') => {
    ...
    it('uses CLAUDE_PLUGIN_ROOT as a fallback when the primary source is missing', () => {
        ...
        const targetPath = join(workDir, 'target', 'SKILL.md');
        const result = installOmcReferenceSkill(targetPath, join(workDir, 'missing-primary'));
        expect(result.installed).toBe(true);
...
//# sourceMappingURL=omc-reference.test.js.map