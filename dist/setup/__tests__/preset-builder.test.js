...
describe('buildPreset — validation failures', () => {
    ...
    it('installStyle=preserve + target=local → throws', () => {
        expect(() => buildPreset({ target: 'local', installStyle: 'preserve' })).toThrow(/installStyle=preserve only valid with target=global/);
    });
    it('installStyle=invalid → throws', () => {
        expect(() => buildPreset({
            installStyle: 'rebuild',
...
//# sourceMappingURL=preset-builder.test.js.map