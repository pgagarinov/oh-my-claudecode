...
if (process.platform === 'win32') {
    ...
    describe('omc-setup skill CLI chain', () => {
        it('check-state → build-preset → preset: writes validated preset and runs it', () => {
            ...
            // ── Step 3: --build-preset ───────────────────────────────────────
            const build = runCli(['--build-preset', '--answers', answersPath, '--out', presetPath], env);
            ...
            const exec = runCli(['--preset', presetPath, '--quiet', '--no-plugin'], env);
            expect(exec.exitCode, `preset run failed — stdout: ${exec.stdout}\nstderr: ${exec.stderr}`).toBe(0);
        });
...
//# sourceMappingURL=skill-omc-setup.e2e.test.js.map