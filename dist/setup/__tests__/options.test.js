...
import { DEFAULTS, InvalidOptionsError, QUESTION_METADATA, loadPreset, parseFlagsToPartial, readEnvPartial, resolveOptions, } from '../options.js';
...
describe('QUESTION_METADATA', () => {
    ...
    it('target question text is verbatim from phase 1', () => {
        expect(QUESTION_METADATA.target.question).toBe('Where should I configure oh-my-claudecode?');
    });
    it('executionMode question text is verbatim from phase 2', () => {
        expect(QUESTION_METADATA.executionMode.question).toBe("Which parallel execution mode should be your default when you say 'fast' or 'parallel'?");
    });
...
describe('parseFlagsToPartial', () => {
    ...
    it('--execution-mode invalid → throws InvalidOptionsError', () => {
        expect(() => parseFlagsToPartial(['--execution-mode', 'bogus'])).toThrow(InvalidOptionsError);
    });
    ...
    it('--mcp-servers with unknown name → throws', () => {
        expect(() => parseFlagsToPartial(['--mcp-servers', 'context7,bogus'])).toThrow(/unknown MCP server: bogus/);
    });
    ...
    it('--exa-key-file missing → X10 error', () => {
        expect(() => parseFlagsToPartial(['--exa-key-file', '/nonexistent/path'])).toThrow(/exa key file not found/);
    });
    ...
    it('--github-token-file missing → error', () => {
        expect(() => parseFlagsToPartial(['--github-token-file', '/nonexistent/path'])).toThrow(/github token file not found/);
    });
    ...
    it('--team-agents invalid count → throws', () => {
        expect(() => parseFlagsToPartial(['--team-agents', '4'])).toThrow(/invalid --team-agents/);
    });
    ...
    it('--skip-hooks forwarded to installerOptions.skipHooks', () => {
        ...
        expect(p.installerOptions?.skipHooks).toBe(true);
    });
...
describe('loadPreset', () => {
    ...
    it('extra unknown fields are preserved (passthrough)', () => {
        ...
        writeFileSync(presetPath, JSON.stringify({ target: 'local', futureField: 'someValue' }), 'utf-8');
        expect(() => loadPreset(presetPath)).not.toThrow();
    });
    it('X8: --preset nonexistent → preset file not found', () => {
        expect(() => loadPreset(join(TMP, 'does-not-exist.json'))).toThrow(/preset file not found/);
    });
...
describe('resolveOptions — precedence', () => {
    ...
    it('preset beats default', () => {
        const opts = resolveOptions({}, { target: 'global', installStyle: 'preserve' }, { env: {}, isTTY: true });
        expect(opts.target).toBe('global');
        expect(opts.installStyle).toBe('preserve');
    });
    it('env beats preset', () => {
        const opts = resolveOptions({}, { target: 'local' }, { env: { OMC_SETUP_TARGET: 'global' }, isTTY: false });
        expect(opts.target).toBe('global');
    });
    it('flag beats env', () => {
    ...
    it('flag beats preset and env (full stack)', () => {
        ...
        const opts = resolveOptions(flags, { executionMode: 'autopilot' }, { env: { OMC_SETUP_EXECUTION_MODE: 'ultrawork' }, isTTY: false });
...
describe('X1–X12 illegal combinations', () => {
    it('X1: --local --global → "conflicting targets"', () => {
        ...
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/conflicting targets: --local and --global/);
    });
    it('X2: --preserve without --global → "--preserve only valid with --global"', () => {
        const flags = parseFlagsToPartial(['--preserve']);
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/--preserve only valid with --global/);
    });
    ...
    it('X3: --wizard + non-TTY + no --preset → "--wizard requires a TTY or --preset"', () => {
        const flags = parseFlagsToPartial(['--wizard']);
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: false })).toThrow(/--wizard requires a TTY or --preset <file>/);
    });
    it('X3: --wizard + non-TTY + --preset → accepted', () => {
        ...
        expect(() => resolveOptions(flags, preset, { env: {}, isTTY: false })).not.toThrow();
    });
    it('X4: --interactive + non-TTY → "--interactive requires a TTY"', () => {
        const flags = parseFlagsToPartial(['--interactive']);
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: false })).toThrow(/--interactive requires a TTY/);
    });
    it('X5: --non-interactive + claude-md phase + missing target + no default → "missing field target"', () => {
        ...
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/missing field target/);
    });
    ...
    it('X6: --mcp-only without --preset and no --mcp-servers → "--mcp-only requires"', () => {
        ...
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/--mcp-only requires --preset <file> or --mcp-servers <list>/);
    });
    ...
    it('X10: --exa-key-file nonexistent → "exa key file not found"', () => {
        expect(() => parseFlagsToPartial(['--exa-key-file', '/does/not/exist'])).toThrow(/exa key file not found/);
    });
    it('X11: --state-save with non-numeric step → "--state-save requires --step <n>"', () => {
        expect(() => parseFlagsToPartial(['--state-save', 'notanumber'])).toThrow(/--state-save requires --step <n>/);
    });
    it('X12: --check-state + --wizard → "mutually exclusive with other phase flags"', () => {
        const flags = parseFlagsToPartial(['--check-state', '--wizard']);
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/--check-state is mutually exclusive with other phase flags/);
    });
    it('X12: --check-state + --claude-md-only → throws', () => {
        const flags = parseFlagsToPartial(['--check-state', '--claude-md-only']);
        expect(() => resolveOptions(flags, undefined, { env: {}, isTTY: true })).toThrow(/--check-state is mutually exclusive/);
    });
