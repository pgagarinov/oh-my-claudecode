...
import { acquireLock, isPidAlive, LockHeldError, STALE_LOCK_MS, } from '../lockfile.js';
function makeTmpDir() {
...
describe('lockfile', () => {
    ...
    describe('acquireLock — conflict handling', () => {
        it('throws LockHeldError on foreign hostname (NFS concern)', () => {
            ...
            expect(() => acquireLock(lockPath, 'cli', { hostname: 'myhost', pid: 1, now: Date.now })).toThrow(LockHeldError);
            try {
        ...
        it('throws LockHeldError when same-host PID is alive', () => {
            ...
            const aliveFn = (pid) => pid === 42;
            expect(() => acquireLock(lockPath, 'cli', {
        ...
        it('auto-releases stale lock (old + dead PID)', () => {
            ...
            const aliveFn = (pid) => pid === 42; // 9999 is dead
            const handle = acquireLock(lockPath, 'cli', {
        ...
        it('does NOT auto-release old lock if PID is still alive', () => {
            ...
            const aliveFn = (pid) => pid === 42; // still alive
            expect(() => acquireLock(lockPath, 'cli', {
        ...
        it('corrupt lockfile with bad timestamp is treated as stale', () => {
            ...
            const aliveFn = () => true; // still auto-releases because timestamp is corrupt
            const handle = acquireLock(lockPath, 'cli', {
