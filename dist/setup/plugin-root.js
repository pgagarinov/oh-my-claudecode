...
function findPluginRootAncestor(start) {
    ...
    for (let i = 0; i < 32; i++) {
        if (existsSync(join(current, 'docs', 'CLAUDE.md')))
            return current;
        if (current === fsRoot)
            return null;
 ...
 */
const DEFAULT_SCRIPT_PLUGIN_ROOT = (() => {
...
function listCacheVersions(base) {
    try {
...
function findLatestValidCacheVersion(cacheBase) {
    const valid = listCacheVersions(cacheBase).filter(v => isValidPluginRoot(join(cacheBase, v)));
    valid.sort(compareVersionsDesc);
    return valid[0] ?? null;
}
function readActiveInstallPath(installedPluginsFile) {
    ...
    if (typeof parsed !== 'object' || parsed === null)
        return null;
    const maybeWrapped = parsed.plugins;
 ...
 */
export function resolveActivePluginRoot(opts = {}) {
    ...
    if (existsSync(cacheBase)) {
        ...
        if (latest) {
            ...
            if (isValidPluginRoot(candidate))
                return candidate;
        }
    }
